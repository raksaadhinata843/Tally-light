...
void checkHubConnection() {
  ...
  if (!isRegisteredWithHub) {
    ...
    if (timeSinceLastAttempt < MIN_RECONNECTION_INTERVAL) return;
...
void handleUDPMessages() {
  ...
  JsonDocument doc;
  if (deserializeJson(doc, incomingPacket) != DeserializationError::Ok) return;
  ...
  if (type == "tally") {
    if (doc["data"].is<JsonObject>()) {
      ...
      if (isAssigned && assignedSource.length() > 0 && sourceId == assignedSource) {
        ...
        if (customDisplayName.length() == 0) currentSource = cleanSourceName(sourceName);
        if (program) updateStatus("PROGRAM");
        else if (preview) updateStatus("PREVIEW");
    ...
    hubConnectionAttempts = 0;
    if (!isAssigned || assignedSource.length() == 0) updateStatus("READY");
    showingRegistrationStatus = true;
    registrationStatusStart = millis();
...
void handleSaveDisplayName() {
  ...
  if (customDisplayName.length() > 0) currentSource = customDisplayName;
  else if (assignedSource.length() > 0) currentSource = cleanSourceName(assignedSource);
  else currentSource = "";
  saveConfiguration();
